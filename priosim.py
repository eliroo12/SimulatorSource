            ...
            if stillinopener:
                ...
                if nextgcd == self.clock:
                    if standarddancing or technicaldancing:
                        ...
                        stepsneeded = 4
                    elif self.buffs['Technical Finish'].getactive(self.clock):
                        proccount = self.countprocs()
                        procgcd = proccount * self.gcd
                        if self.buffs['Flourishing Fountain'].getactive(self.clock) and self.buffs['Flourishing Fountain'].closetodrop(self.clock,procgcd):
                            self.buildpotency(
                                self.abilities['Fountainfall'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                          self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Flourishing Bladeshower'].getactive(self.clock) and self.buffs['Flourishing Bladeshower'].closetodrop(self.clock,procgcd):
                            self.buildpotency(
                                self.abilities['Bloodshower'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                             self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Flourishing Cascade'].getactive(self.clock) and self.buffs['Flourishing Cascade'].closetodrop(self.clock,procgcd):
                            self.buildpotency(
                                self.abilities['Reverse Cascade'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                             self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Flourishing Windmill'].getactive(self.clock)and self.buffs['Flourishing Cascade'].closetodrop(self.clock,procgcd):
                            self.buildpotency(
                                self.abilities['Rising Windmill'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                             self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.abilities['Standard Step'].available(self.clock):# and self.buffs['Devilment'].returnduration(self.clock) > 3.6:
                            if self.createlog:
                                logging.info(str(self.clock) + ' : You begin Standard Step')
                            nextgcd = round(self.clock + 1.5, 2)
                            self.abilities['Standard Step'].putonCD(self.clock)
                            ...
                            nextgcd = round(self.clock + 1.5, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Combo'].getactive(self.clock) and self.buffs['Combo'].closetodrop(self.clock,self.gcd):  # If we are in combo we are doing self.abilities['Fountain'] here
                            self.buildpotency(
                                self.abilities['Fountain'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                      self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Flourishing Fountain'].getactive(self.clock):
                            self.buildpotency(
                                self.abilities['Fountainfall'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                          self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            if self.checkproc():
                                self.buffs['Flourishing Fountain'].activate(self.clock)
                                self.schedule.addtime(self.buffs['Flourishing Fountain'].activation)
                        elif self.buffs['Flourishing Cascade'].getactive(self.clock):
                            self.buildpotency(
                                self.abilities['Reverse Cascade'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                             self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Flourishing Bladeshower'].getactive(self.clock):
                            self.buildpotency(
                                self.abilities['Bloodshower'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                             self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                            ...
                            nextgcd = round(self.clock + self.gcd, 2)
                            nextaction = round(self.clock + self.abilitydelay, 2)
                        elif self.buffs['Flourishing Windmill'].getactive(self.clock):
                            self.buildpotency(
                                self.abilities['Rising Windmill'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                         self.buffs['Combo'].getactive(self.clock)))
                            gcd = gcd + 1
                        ...
                        if self.abilities['Standard Step'].available(
                            ...
                            if self.esprit > 40 and delayend - delaystart > 20 and delaystart < self.clock + self.gcd and \
                                ...
                                self.esprit = self.esprit - self.abilities['Saber Dance'].cost
                                self.buildesprit(self.rate)
                            elif self.esprit > 49 and not 'NIN' in self.party.keys() and self.abilities['Flourish'].available(nextgcd) and self.countprocs() == 0 and self.abilities['Technical Step'].getrecast(self.clock)>30:
                                self.buildpotency(
                                    self.abilities['Saber Dance'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                             self.buffs['Combo'].getactive(self.clock)))
                                saberdancess = saberdancess + 1
                    ...
                    if technicaldancing or standarddancing:
                        ...
                        nextaction = round(nextgcd - 1, 2)
                            ...
                            self.buffs['Flourishing Fan'].getactive(self.clock)):
                        if self.createlog:
                            logging.info(str(self.clock) + ' : You use Flourish!')
                        self.buffs['Flourishing Cascade'].activate(self.clock)
                        self.buffs['Flourishing Bladeshower'].activate(self.clock)
