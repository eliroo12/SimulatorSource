...
class sim:
    ...
    def __init__(self, number, length, open, fight, stats, abilities, party, pbuffs, buffs, dots, potion, astpriority,
                 dncpartner, log):

        logging.basicConfig(filename='BRD_Sim_Log' + str(number), filemode='w', format='%(message)s',
                            level=logging.INFO)
        self.length = length
        self.opener = open
        self.fight = fight
        ...
        self.abilities['Bloodletter'].charged = True
        self.abilities['Bloodletter'].charges = 3
        self.abilities['Bloodletter'].maxcooldown = 3 * 15
    ...
    def handlerep(self):
        if self.buffs['Minuet'].getactive(self.clock):
            ...
            if self.abilities['Bloodletter'].returncharges(self.clock) > 2:
                if self.createlog:
                    logging.info(str(self.clock) + ' You lost a proc')
            else:
    ...
    def songlogic(self, songname, nexttick, delaystart):
        if songname == 'Minuet':
            if self.abilities['Minuet'].available(self.clock) and (
                    not self.buffs['Ballad'].getactive(self.clock) or self.buffs['Ballad'].endtime < nexttick):
                return True
            else:
                return False
        elif songname == 'Ballad':
            if self.abilities['Ballad'].available(self.clock) and (
                    not self.buffs['Minuet'].getactive(self.clock) or self.buffs['Minuet'].endtime < nexttick):
                return True
            else:
                return False
        else:
            if self.abilities['Paeon'].available(self.clock) and (
                    not self.buffs['Ballad'].getactive(self.clock) or self.buffs['Ballad'].endtime < nexttick) and (
                    not self.buffs['Minuet'].getactive(self.clock) or self.buffs['Minuet'].endtime < nexttick):
                return True
            else:
                return False
        ...
        if not future:
            if self.abilities['Empyreal Arrow'].available(self.clock):
                if self.buffs['Minuet'].getactive(self.clock) and self.rep > 2:
                    return False
                elif self.buffs['Minuet'].getactive(
                        self.clock) and self.rep > 2 and self.clock - nexttick < self.abilitydelay:
                    return False
                elif self.buffs['Ballad'].getactive(self.clock) and self.abilities['Bloodletter'].returncharges(self.clock) > 2:
                    return False
            ...
            if self.abilities['Empyreal Arrow'].available(nextgcd):
                if self.buffs['Minuet'].getactive(nextgcd) and self.rep > 2:
                    return False
                        ...
                        nextgcd) and self.rep > 1 and nextgcd - nexttick < self.abilitydelay:
                    return False
                elif self.buffs['Ballad'].getactive(nextgcd) and  self.abilities['Bloodletter'].returncharges(self.clock) > 2:
                    return False
    ...
    def cliplogic(self, nexttick):
        if self.buffs['Minuet'].getactive(self.clock) and self.buffs['Minuet'].closetodrop(self.clock,
                                                                                           self.gcd) and self.rep > 0 and nexttick - self.abilitydelay > \
                self.buffs['Minuet'].endtime:
            return True
        elif self.buffs['Ballad'].getactive(self.clock) and  self.abilities['Bloodletter'].returncharges(self.clock) > 2 and nexttick - self.clock < 1.5:
            return True
        else:
            return False
    ...
    def snaplogic(self, potmod):
        ...
        if not self.buffs['Raging Strikes'].getactive(self.clock):
            return False
        if self.buffs['Raging Strikes'].closetodrop(self.clock, self.gcd) and self.dots[
            'Stormbite'].endtime - self.clock < 20:
            return True
        return False
    ...
    def futuresonglogic(self, time, songname, nexttick, delaystart):
        if songname == 'Minuet':
            ...
            if self.abilities['Paeon'].available(time) and (
                    not self.buffs['Ballad'].getactive(time) or self.buffs['Ballad'].endtime < nexttick) and (
                    not self.buffs['Minuet'].getactive(time) or self.buffs['Minuet'].endtime < nexttick):
                return True
            else:
                return False
    ...
    async def sim(self):
        ...
        while self.clock < self.length:
            ...
            elif self.buffs['Raging Strikes'].getactive(self.clock):
                buffwindow = True
                lastbuffwindow = self.clock
            ...
            if self.clock == nextauto:
                if not delay:
                    self.buildpotency(
                        self.abilities['Auto Attack'].getpotency(self.clock, CDHStats, automod, DMGStats, True))
            ...
            if stillinopener:
                if self.action[posinopen].actionable(self.clock):
                    ...
                    if currentaction.name == 'Bloodletter':
                        ...
                        if self.abilities['Empyreal Arrow'].available:
                            self.buildpotency(
                                self.abilities['Empyreal Arrow'].getpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                            True))
                            self.handlerep()
                    elif currentaction.name == 'Caustic Bite':
                        self.buildpotency(
                            self.abilities['Caustic Bite'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        ...
                        self.buildpotency(
                            self.abilities['Stormbite'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        ...
                        self.buildpotency(
                            self.abilities['Iron Jaws'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        self.dots['Stormbite'].activate(self.clock, CDHStats, potmod, DMGStats)
                        self.dots['Caustic Bite'].activate(self.clock, CDHStats, potmod, DMGStats)
                        self.schedule.addtime(self.dots['Stormbite'].activation)
                        self.schedule.addtime(self.dots['Caustic Bite'].activation)
                        ...
                        if self.abilities['Minuet'].available(self.clock):
                            self.buildpotency(
                                self.abilities['Minuet'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        ...
                        if self.buffs['Minuet'].getactive(self.clock) and self.rep > 2:
                            if self.abilities['Pitch Perfect'].available(self.clock):
                                self.buildpotency(
                                    self.abilities['Pitch Perfect'].pitchpotency(self.clock, CDHStats, potmod, DMGStats,
                                                                                 True, self.rep))
                                self.rep = 0
                ...
                if nextgcd == self.clock:
                    ...
                    if self.buffs['Barrage'].getactive(self.clock) and self.buffs['SS Ready'].getactive(self.clock):
                        ...
                        nextgcd = round(self.clock + self.gcd, 2)
                        nextaction = round(self.clock + self.abilitydelay, 2)
                    elif self.dots['Caustic Bite'].getactive(self.clock) and self.dots['Stormbite'].getactive(
                            self.clock) and self.snaplogic(potmod):
                        self.buildpotency(
                            self.abilities['Iron Jaws'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        self.dots['Caustic Bite'].activate(self.clock, CDHStats, potmod, DMGStats)
                        self.dots['Stormbite'].activate(self.clock, CDHStats, potmod, DMGStats)
                        ...
                        nextaction = round(self.clock + self.abilitydelay, 2)
                            ...
                            self.clock) and (self.dots['Caustic Bite'].closetodrop(self.clock, self.gcd) or self.dots[
                        'Stormbite'].closetodrop(self.clock, self.gcd)):
                        self.buildpotency(
                            self.abilities['Iron Jaws'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        self.dots['Caustic Bite'].activate(self.clock, CDHStats, potmod, DMGStats)
                        self.dots['Stormbite'].activate(self.clock, CDHStats, potmod, DMGStats)
                        ...
                        nextgcd = round(self.clock + self.gcd, 2)
                        nextaction = round(self.clock + self.abilitydelay, 2)
                    elif self.buffs['SS Ready'].getactive(self.clock) and self.buffs['SS Ready'].closetodrop(self.clock,
                                                                                                             self.gcd):
                        self.buildpotency(
                        ...
                        self.buildpotency(
                            self.abilities['Apex Arrow'].apexpotency(self.clock, CDHStats, potmod, DMGStats, True,
                                                                     self.soulvoice))
                        self.soulvoice = 0
                        nextgcd = round(self.clock + self.gcd, 2)
                        nextaction = round(self.clock + self.abilitydelay, 2)
                    ...
                    abilityused = False
                    if clip and self.buffs['Minuet'].getactive(self.clock) and self.abilities[
                        'Pitch Perfect'].available(self.clock) and self.rep > 0:
                        self.buildpotency(
                        ...
                        clip = False
                        abilityused = True
                    elif clip and self.buffs['Ballad'].getactive(self.clock) and self.abilities[
                        'Bloodletter'].available(self.clock):
                        self.buildpotency(
                            self.abilities['Bloodletter'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        nextaction = round(self.clock + self.abilitydelay, 2)
                        clip = False
                        abilityused = True
                    elif not clip and self.abilities['Bloodletter'].returncharges(self.clock) > 2:
                        self.buildpotency(
                            self.abilities['Bloodletter'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        abilityused = True
                    elif not clip and self.buffs['Ballad'].getactive(self.clock) and self.abilities[
                        'Bloodletter'].returncharges(self.clock) > 1 and nexttick - self.clock < 1.4:
                        self.buildpotency(
                            self.abilities['Bloodletter'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        abilityused = True
                    elif self.buffs['Minuet'].getactive(self.clock) and self.rep > 2 and self.abilities[
                        'Pitch Perfect'].available(self.clock):
                        self.buildpotency(
                            self.abilities['Pitch Perfect'].pitchpotency(self.clock, CDHStats, potmod, DMGStats, True,
                                                                         self.rep))
                        self.rep = 0
                        nextaction = round(self.clock + self.abilitydelay, 2)
                        abilityused = True
                    elif self.buffs['Minuet'].getactive(self.clock) and self.rep > 0 and self.buffs[
                        'Minuet'].endtime < nextgcd and self.abilities['Pitch Perfect'].available(self.clock):
                        self.buildpotency(
                            self.abilities['Pitch Perfect'].pitchpotency(self.clock, CDHStats, potmod, DMGStats, True,
                                                                         self.rep))
                        self.rep = 0
                        ...
                        self.buildpotency(
                            self.abilities['Ballad'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        ...
                        self.buildpotency(
                            self.abilities['Paeon'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        self.buffs['Paeon'].activate(self.clock)
                        self.schedule.addtime(self.buffs['Paeon'].activation)
                        abilityused = True
                    elif not clip and self.abilities['Raging Strikes'].available(
                            self.clock) and nextgcd - self.clock < 1.5 and not self.buffs['Paeon'].getactive(
                            self.clock):
                        self.abilities['Raging Strikes'].putonCD(self.clock)
                        self.buffs['Raging Strikes'].activate(self.clock)
                        self.schedule.addtime(self.buffs['Raging Strikes'].activation)
                        ...
                        self.buildpotency(
                            self.abilities['Empyreal Arrow'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        nextaction = round(self.clock + self.abilitydelay, 2)
                        self.handlerep()
                        abilityused = True
                    elif not clip and self.abilities['Barrage'].available(self.clock) and not self.buffs[
                        'SS Ready'].getactive(self.clock) and not (
                            self.dots['Caustic Bite'].closetodrop(self.clock, self.gcd) or self.dots[
                        'Stormbite'].closetodrop(self.clock, self.gcd)):
                    ...
                    elif buffwindow and self.abilities['Bloodletter'].available(self.clock):
                        self.buildpotency(
                            self.abilities['Bloodletter'].getpotency(self.clock, CDHStats, potmod, DMGStats, True))
                        nextaction = round(self.clock + self.abilitydelay, 2)
                        abilityused = True
                            ...
                            self.abilities['Raging Strikes'].getrecast(self.clock) < 10):
                        if self.createlog:
                            logging.info(str(self.clock) + ' : You use a potion!')
                        self.buffs['Potion'].activate(self.clock)
                        self.schedule.addtime(self.buffs['Potion'].activation)
                    ...
                    if abilityused:  # Process new action time
                        ...
                        if nextaction > nextgcd:
                            ...
                            if not self.cliplogic(nexttick):
                                ...
                                if self.buffs['Minuet'].getactive(self.clock) and not self.abilities[
                                    'Pitch Perfect'].available(self.clock) and self.abilities[
                                    'Pitch Perfect'].nextuse > nextaction and self.abilities['Pitch Perfect'].nextuse < \
                                        self.buffs['Minuet'].endtime:
                                    nextaction = self.abilities['Pitch Perfect'].nextuse
                                clip = True
                        self.abilitydelay = .7
                    else:  ### Future Schedule Logic
                        if not self.abilities['Bloodletter'].available(self.clock) and self.abilities[
                            'Bloodletter'].getrecast(self.clock) <= nextgcd - self.clock - self.abilitydelay:
                            nextaction = round(self.abilities['Bloodletter'].nextuse, 2)
                        elif not self.abilities['Barrage'].available(self.clock) and self.abilities[
                            'Barrage'].getrecast(self.clock) <= nextgcd - self.clock - self.abilitydelay:
                            nextaction = round(self.abilities['Barrage'].nextuse, 2)
                        elif not self.abilities['Battle Voice'].available(self.clock) and self.abilities[
                            'Battle Voice'].getrecast(self.clock) <= nextgcd - self.clock - self.abilitydelay and not (
                                self.dots['Caustic Bite'].closetodrop(self.clock, self.gcd) or self.dots[
                            'Stormbite'].closetodrop(self.clock, self.gcd)):
                            nextaction = round(self.abilities['Battle Voice'].nextuse, 2)
                        elif not self.abilities['Raging Strikes'].available(self.clock) and self.abilities[
                            'Raging Strikes'].getrecast(self.clock) <= nextgcd - self.clock - self.abilitydelay and not \
                        self.buffs['Paeon'].getactive(self.clock) and self.abilities[
                            'Raging Strikes'].nextuse - self.clock < 1.5:
                            nextaction = round(self.abilities['Raging Strikes'].nextuse, 2)
                        elif not self.abilities['Empyreal Arrow'].available(self.clock) and self.abilities[
                            'Empyreal Arrow'].available(nextgcd) and self.ealogic(nexttick, True, nextgcd):
            ...
            if (oldpot != self.potency) or (oldsv != self.soulvoice) or (oldrep != self.rep):
                if self.createlog:
                    logging.info(str(self.clock) + ' : Potency: ' + str(round(self.potency, 1)) + ' ' + str(
                        potmod) + ' || Soul Voice: ' + str(
                        self.soulvoice) + ' || Repertoire: ' + str(self.rep) + ' || Crit Rate: ' + str(
                        CDHStats[0]) + ' || DH Rate: ' + str(CDHStats[2])+ ' || BL Charges: '+str(self.abilities['Bloodletter'].returncharges(self.clock)))
                oldpot = self.potency
