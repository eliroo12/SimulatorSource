...
async def runsim(runjobs,runtime,currentid,open,fight,stattable,abilities,party,pbuffs,buffs,dots,potion,ast,dncpart,log):
    tasks = []
    tasks2 = []
    tasks3 = []
    a = []
    ...
    while x < runjobs:
        one = True
        tasks.append(asyncio.create_task(sim(1, runtime, open, fight, stattable, abilities, party, pbuffs, buffs,dots, potion,ast,dncpart, log).sim()))
        x = x + 1
        if x < runjobs:
            two = True
            tasks2.append(asyncio.create_task(sim(1, runtime, open,fight, stattable, abilities, party, pbuffs, buffs,dots, potion,ast,dncpart, log).sim()))
        x = x + 1
        if x < runjobs:
            three = True
            tasks3.append(asyncio.create_task(sim(1, runtime,  open, fight, stattable, abilities, party, pbuffs, buffs,dots, potion,ast,dncpart, log).sim()))
        x = x + 1
    if one:
...
async def run1sim(runjobs,runtime,currentid,open,fight,stattable,abilities,party,pbuffs,buffs,dots,potion,ast,dncpart,log):
    tasks = []
    tasks2 = []
    tasks3 = []
    a = []
    ...
    while x < runjobs:
        one = True
        sim1 = sim(1, runtime, open, fight, stattable, abilities, party, pbuffs, buffs,dots, potion,ast,dncpart, log)
        tasks.append(asyncio.create_task(sim1.sim()))
        x = x + 1
        if x < runjobs:
            two = True
            sim2 = sim(1, runtime, open, fight, stattable, abilities, party, pbuffs, buffs,dots, potion,ast,dncpart, log)
            tasks2.append(asyncio.create_task(sim2.sim()))
        x = x + 1
        if x < runjobs:
            three = True
            sim3 = sim(1, runtime, open, fight, stattable, abilities, party, pbuffs, buffs,dots, potion,ast,dncpart, log)
    ...
    if one:
        ...
        for i in dot.keys():
            if i+' Dot' in abilitydict.keys():
                abilitydict[i+' Dot'] = abilitydict[i+' Dot'] + dot[i].totalpotency
            else:
                abilitydict[i+' Dot'] = dot[i].totalpotency
    ...
    if two:
        ...
        for i in ability.keys():
            if i in abilitydict.keys():
                ...
                abilitydict[i] = ability[i].totalpotencye
        for i in dot.keys():
            if i+' Dot' in abilitydict.keys():
                abilitydict[i+' Dot'] = abilitydict[i+' Dot'] + dot[i].totalpotency
            else:
...
def main():
    ...
    while True:
        ...
        if not win2_active and button == 'party':
            window.Hide()
            win2_active = True
            layout2 = [[sg.Text('Job', size=(8,1)),sg.Text('Member',size=(8,1)), sg.Text('Buff Priority',size=(8,1))],]
            for i in jobs.values():
                ...
                if i.name == 'AST':
                    ...
                    row.append(sg.Checkbox('Partner', size=(8, 1), default=tether, key='partner'))
                layout2.append(row)
                ...
                if simtwoaction:
                    ...
                    while i > 0:
                        ...
                        if i > 5:
                            ...
                            jobruns = i
                        results = asyncio.run(runsim(jobruns,runlength,i,opener1,fight,simonetable,abilities,party,pbuffs,buffs,dots,potion,astpriority,dncpart,logging))
                    ...
                    while i > 0:
                        ...
                        results = asyncio.run(
                            runsim(jobruns, runlength, i, opener2, fight, simtwotable, abilities, party, pbuffs,
                                   buffs,dots, potion, astpriority,dncpart, logging))
                    ...
                    while i > 0:
                        ...
                        if i > 5:
                            ...
                            jobruns = i
                        results, times, ability = asyncio.run(run1sim(jobruns, runlength, (runtimes-i), opener1, fight, simonetable, abilities, party, pbuffs,buffs,dots, potion, astpriority,dncpart, logging))
                        potency1 = potency1 + results
                        for u in times:
                            viewtimes.append(u)
